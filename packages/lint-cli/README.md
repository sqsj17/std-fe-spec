...
在项目根目录执行 `std-lint-cli init`，即可一键接入规范，为项目安装规范 `Lint` 所需的依赖和配置。

具体会做以下事情：

- 安装各种依赖：包括 `Linter` 依赖，如 [ESLint](https://eslint.org/)、[stylelint](https://stylelint.io/)、[commitlint](https://commitlint.js.org/#/)、[markdownlint](https://github.com/DavidAnson/markdownlint) 等；配置依赖，如 [std-eslint-config](https://www.npmjs.com/package/en-eslint-config)、[std-stylelint-config](https://www.npmjs.com/package/std-stylelint-config)、[std-commitlint-config](https://www.npmjs.com/package/std-commitlint-config)、[std-markdownlint-config](https://www.npmjs.com/package/std-markdownlint-config) 等
- 写入各种配置文件，包括：
  - `.eslintrc.js`、`.eslintignore`：ESLint 配置（继承 `std-eslint-config`）及黑名单文件
  - `.stylelintrc.js`、`.stylelintignore`：stylelint 配置（继承 `std-stylelint-config`）及黑名单文件
  - `commitlint.config.js`：commitlint 配置（继承 `std-commitlint-config`）
...
>
> 注 2：如果项目的 .vscode/ 目录被 .gitignore 忽略，可以在拉取项目后单独执行 `std-lint-cli init --vscode` 命令写入 `.vscode/extensions.json` 和 `.vscode/settings.json` 配置文件

#### `std-lint-cli scan`：一键扫描
...
> 注 1：事实上，你可以在任意目录执行 `std-lint-cli scan` `std-lint-cli` 会根据文件类型、JSON 等特征嗅探项目类型。但我们还是推荐在执行过 `std-lint-cli init` 的项目根目录执行 `std-lint-cli scan`，以得到最准确的扫描结果。
>
...
#### `std-lint-cli commit-file-scan` 提交文件扫描
...
#### `std-lint-cli commit-msg-scan` 提交信息扫描
...
- std-lint-cli.init(config)：将项目一键接入规范，效果等同于 `std-lint-cli init`
...
`std-lint-cli` 基于一份配置进行扫描（但你也可以零配置使用），支持的配置参数有：
...
`std-lint-cli` 会读取执行目录下的 `std-lint-cli.config.js` 作为配置文件。`std-lint-cli init` 会在执行目录下新增如下的 `std-lint-cli.config.js` 文件：
...
如果你的 TS 项目 commit 卡口和 `std-lint-cli scan` 运行时间很长，可以通过如下在 `.eslintrc.js` 中增加以下配置提升性能：
